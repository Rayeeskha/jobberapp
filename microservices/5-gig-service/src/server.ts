...
const SERVER_PORT = Number(config.SERVER_PORT) || 4004;
  ...
  try {
    ...
    httpServer.on('error', (error: NodeJS.ErrnoException) => {
      if (error.code === 'EADDRINUSE') {
        log.log(
          'error',
          `Gig server port ${SERVER_PORT} is already in use. Set SERVER_PORT to a free port or stop the process using ${SERVER_PORT}.`,
        );
        return;
      }

      log.log('error', 'GigService HTTP server error:', error);
    });
