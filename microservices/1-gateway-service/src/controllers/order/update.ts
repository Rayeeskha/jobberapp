...
export class Update {
  public async cancel(req: Request, res: Response): Promise<void> {
    ...
    const response: AxiosResponse = await orderService.cancelOrder(paymentIntentId, String(orderId), orderData);
  ...
  public async requestExtension(req: Request, res: Response): Promise<void> {
    const { orderId } = req.params;
    const response: AxiosResponse = await orderService.requestDeliveryDateExtension(String(orderId), req.body);
  ...
  public async deliveryDate(req: Request, res: Response): Promise<void> {
    ...
    const response: AxiosResponse = await orderService.updateDeliveryDate(String(orderId), String(type), req.body);
    res.status(StatusCodes.OK).json({ message: response.data.message, order: response.data.order });
  }

  public async deliverOrder(req: Request, res: Response): Promise<void> {
    const { orderId } = req.params;
    const response: AxiosResponse = await orderService.deliverOrder(String(orderId), req.body);
    res.status(StatusCodes.OK).json({ message: response.data.message, order: response.data.order });
  }

  public async approveOrder(req: Request, res: Response): Promise<void> {
    const { orderId } = req.params;
    const response: AxiosResponse = await orderService.approveOrder(String(orderId), req.body);
    res.status(StatusCodes.OK).json({ message: response.data.message, order: response.data.order });
  }
